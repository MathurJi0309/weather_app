...
const Input = ({setQuery,setCelsius}) => {
  ...
  return (
    <div className="flex felx-row justify-center my-6 space-x-8">
      ...
      <div className="flex flex-row w=1/4 items-center justify-center space-x-3">
        <button name="metric"  className="text-xl text-white font light transition ease-out hover:scale-125"  onClick={()=>setCelsius(true)} >°C</button>
        <button name="imperial" className="text-xl text-white font light transition ease-out hover:scale-125" onClick={()=>setCelsius(false)}>°F</button>
